...
class SyncService:
    def __init__(self):
        self.main_records=[]  """to store the data"""
    ...
    def onMessage(self, data: dict):
        ...
        if date['type']=='probe':     """checking the type of received message"""
            return{'type': 'update', 'data': self.main_records}   """here the type is probe request ,thus the data key (main_records) which contains the servers record is send back to device as update"""
